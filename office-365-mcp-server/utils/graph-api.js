...
const { refreshAccessToken } = require('../auth/auto-refresh');
...
const RETRY_CONFIG = {
  ...
  maxAuthRetries: 1 // Only retry auth once to avoid loops
};
 ...
 * @param {number} authRetryCount - Auth retry attempt (internal use)
 * @returns {Promise<object>} - The API response
 */
async function callGraphAPI(accessToken, method, path, data = null, queryParams = {}, customHeaders = {}, retryCount = 0, authRetryCount = 0) {
      ...
      const req = https.request(url, options, (res) => {
        ...
        res.on('end', () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            if (authRetryCount < RETRY_CONFIG.maxAuthRetries) {
              console.error(`[GRAPH-API] Got 401, attempting token refresh (attempt ${authRetryCount + 1})`);

              refreshAccessToken()
                .then((newTokens) => {
                  console.error('[GRAPH-API] Token refresh successful, retrying request');
                  // Retry with new token
                  callGraphAPI(newTokens.access_token, method, path, data, queryParams, customHeaders, 0, authRetryCount + 1)
                    .then(resolve)
                    .catch(reject);
                })
                .catch((refreshError) => {
                  console.error('[GRAPH-API] Token refresh failed:', refreshError.message);
            ...
            if (shouldRetry) {
              ...
              // Wait and retry
              setTimeout(() => {
                callGraphAPI(accessToken, method, path, data, queryParams, customHeaders, retryCount + 1, authRetryCount)
                  .then(resolve)
                  .catch(reject);
              }, delay);
              return;
