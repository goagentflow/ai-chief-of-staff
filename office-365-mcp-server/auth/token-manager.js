...
/**
 * Performs token refresh with locking to prevent concurrent refreshes
 * @returns {Promise<object>} - New tokens
 */
async function doLockedRefresh() {
...
async function getAccessToken(autoRefresh = true) {
  ...
  if (cachedTokens && cachedTokens.access_token) {
    if (autoRefresh && needsRefresh(cachedTokens)) {
      ...
      try {
        const newTokens = await doLockedRefresh();
        return newTokens.access_token;
      } catch (error) {
        console.error('[TOKEN-MANAGER] Auto-refresh failed:', error.message);
        // Don't return expired token - throw so caller knows refresh failed
        if (Date.now() > cachedTokens.expires_at) {
          throw new Error('Token expired and refresh failed: ' + error.message);
        }
        // Token not yet expired, can still use it briefly
